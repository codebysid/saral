...
const MetricCard = ({ count, name: title }: IMetricCard) => {
    ...
    return (
        <div className=" flex gap-2 p-[16px] border border-custom-border rounded-[8px]">
            <div className={`${currentIcon.style} h-[54px] w-[54px] rounded-full flex justify-center items-center`}>
                <Icons name={currentIcon.icon as IconName} />
            </div>

            <div className=" flex flex-col">
                <span className=" text-[22px] font-medium text-[#202224]">
                    {count}
                </span>
                <span className="subHeadline text-[16px]">{title}</span>
            </div>
        </div>
    )
}
